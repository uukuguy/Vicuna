...
def train(args):
    ...
    model, tokenizer = get_data_model(args)
    ...
    cache_train_dir = f"{cache_dir}/train_data"
    cache_val_dir = f"{cache_dir}/val_data"
    if os.path.exists(cache_train_dir):
        train_data = datasets.load_from_disk(cache_train_dir)
        if os.path.exists(cache_val_dir):
            val_data = datasets.load_from_disk(cache_val_dir)
        else:
            val_data = None
    else:
        ...
        if args.val_set_size > 0:
            ...
            val_data = None
        train_data.save_to_disk(cache_train_dir)
        if val_data is not None:
            val_data.save_to_disk(cache_val_dir)
